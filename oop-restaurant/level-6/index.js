...
let waiterOrder = function (restaurant) {
  ...
  restaurant.cash = payCoat(restaurant.queue[0].orderList, restaurant.cash);
  logHandler("Cash: " + restaurant.cash, "Restaurant");
...
let customerLeave = function (restaurant) {
  ...
  restaurant.cash = getCash(restaurant.queue[0].orderList, restaurant.cash);
  logHandler("Cash: " + restaurant.cash, "Restaurant");

  textHandler("I'm leaving, pay for you.", "customerToWaiter");
...
// 付出成本
let payCoat = function (orderList, cash) {
  let sum = 0;
  for (let i = 0; i < orderList.length; i++) {
    sum += orderList[i].cost;
  }
  cashDom.innerHTML = '餐厅本金：' + (cash - sum);
  return cash - sum;
};
// 收取金钱
let getCash = function (orderList, cash) {
  let sum = 0;
  for (let i = 0; i < orderList.length; i++) {
...
let timeDom = operate.getElementsByTagName('p')[1];
let cashDom = operate.getElementsByTagName('p')[0];

function timeLoop () {
  let date = new Date();
  timeDom.innerHTML = date.toLocaleTimeString();
  setTimeout(function () {
    timeLoop();
  }, 1000);
}
