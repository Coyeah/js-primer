...
import { hash, session, storage, redirectToLogin, getOtherSystermParams } from './index';
...
export default (...args) => {
  ...
  const { method, body, params, query, cache, force, disableContentType, ...restOptions } = options;
  options = { ...restOptions, method };

  if ((method === 'POST' || method === 'PUT') && !(body instanceof FormData)) {
    let otherSystermparams = getOtherSystermParams('wms', true);
    let data = {
      ...body,
      ...otherSystermparams,
    }
  ...
  if(disableContentType === true) delete options.headers['Content-Type'];
