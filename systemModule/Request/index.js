...
export const getEnv = () => {
  const dev = window.location.hostname.split('.')[1];
  const domain = window.location.hostname.substring(window.location.hostname.indexOf('.'));
  switch (dev) {
    case 'dev': return {name: '_D_W_I_S_ID_', domain};
    case 'qa': return {name: '_Q_W_I_S_ID_', domain};
    case 'hsmob': return {name: '_W_I_S_ID_', domain};
    default: return null;
  }
}

// 从当前url上获取第三方(wms)登录信息
export const getHashQuery = () => {
  const hash = window.location.hash;
  const hashquery = hash.indexOf('?') > 0 ? hash.substring(hash.indexOf('?')) : '';
  return hashquery;
}

export const handleOtherSystermParams = (token, originParams) => {
  if(!originParams || Object.keys(originParams).length <= 0) return {};
  let mapKeys = [];
  switch (token) {
    case 'wms': mapKeys = [['serviceName', 'serviceName'],['authorAccount','serviceAuthorAccount'], ['authorName', 'serviceAuthorName']];
  }
  return mapKeys.reduce((memo, cur) => ({...memo, [cur[1]]: originParams[cur[0]]}), {});
}

export const getOtherSystermParams = (token, handle = false) => {
  const search = getHashQuery();
  if (search) {
    const params = qs.parse(search.substring(1));
    if (params && params.cookie && params.serviceName === token) {
      if (handle) {
        return {...handleOtherSystermParams(token,  params)};
      }
      return {...params};
    }
  }
...
// 角色处理时长,后台返回的是秒数
export const getDelayTime = (time) => {
  const d = 24 * 60 * 60, h = 60 * 60, m = 60;
  const day = Math.floor(time / d),
        hour = Math.floor((time - day * d) / h),
        minute = Math.floor((time - day *d  - hour * h) / m);
  return (day || hour || minute) ? `${day ? day + '天 ' : ''}${hour ? hour + '小时 ' : ''}${minute ? minute + '分钟' : ''}` : 0;
}

// 是否是从其他系统跳转过来的, url后面会带?cookie=12332
export const isFromOtherSysterm = () => {
  const hashSearch = window.location.href.split('?')[1];
  if (hashSearch && qs.parse(hashSearch) && qs.parse(hashSearch).cookie) {
    return true;
  }
  return false;
}
...
export const redirectToLogin = () => {
  ...
  session(null);
  if(redirect.indexOf('auth/login') < 0) {
...
export const toHashPath = (path = '') => `/#${path}`.replace(/\/+/g, '/').replace(/^(\/#)+/g, '/#');
...
export const toTreeData = (treeData = [], { id = 'id', pId = 'parentId', title = 'title', rootPId = 0 }) => {
  const tree = (data, parentId) =>
    data.filter(item => (item[pId] === parentId)).map(node => {
 ...
 * @param {*} list   列表数组
 */
export function changeEmptyValue(list) {
  if(!list) {
    return [];
  }
  list = _.cloneDeep(list.slice());
  list.map((item) => {
